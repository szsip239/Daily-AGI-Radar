...
GitHub 项目不添加风险标签、不因风险剔除；Skills 风险只作标注、不影响收录，详见[收录规则调整](docs/audits/project-risk-review-2026-09-13.md)。Skills 新上榜按 slug 对账并保留每日新增快照。
...
日报内容只选择并生成一次，文字与播客共用同一份内容。播客生成或上传失败只记录警告，继续发布文字日报；云文档等必需文字产物失败仍使相应阶段报错。失败或未验证上传的音频不进入公开音频清单，后续成功重试后恢复。重跑前的音频保留在本地历史目录，未完成的新音频不会覆盖为正式产物。
