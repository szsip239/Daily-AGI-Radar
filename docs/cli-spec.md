...
```json
{
  ...
  "search_keywords": ["AI Agent", "自动化工作流", "GitHub"],
  "search_aliases": ["Agent工具", "流程自动化"]
}
```

`search_keywords` and `search_aliases` are optional public discovery terms generated by the export
pipeline. They keep CLI commands unchanged while improving Chinese broad-topic search recall.
...
- Ranking fuses MiniSearch rank with token-overlap rank, query-part coverage, corpus-aware token weight, recency, and public quality signals.
- Phrase matches, exact key matches, and coverage across multiple query terms may boost deterministic rank.
